...
# 주의: extract_dual_major_slots(bot_core.py)는 학생이 친 표현을 "있는 그대로" 추출하고
# 줄임말을 정식 학과명으로 확장하라는 지시가 없음 — 그래서 "미자공"처럼 흔한 줄임말이
# LLM 훈련 데이터에 우연히 익숙한 표현이면 알아서 정식명으로 풀어써서 넘어오고, 아니면
# 줄임말 그대로 넘어와서 resolve_major가 못 찾는 식으로 "어떨 땐 되고 어떨 땐 안 되는"
# 현상이 생김(실사용자 리포트로 확인). LLM 운에 맡기지 않고, 이 프로젝트 다른 곳과 같은
# 원칙(판단은 코드가 결정론적으로)대로 자주 쓰이는 줄임말을 alias 테이블에 직접 등록해서
# 항상 똑같이 인식되게 함.
MAJOR_ALIASES = {
    "미래자동차공학과": "미래자동차공학과",
    "미래자동차공학": "미래자동차공학과",
